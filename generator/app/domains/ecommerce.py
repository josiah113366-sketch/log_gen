...
'''
import random # 값을 선택, 생성 시 확률적인 정보(선택) 제공하는 도구
...
# 가중치 부여하여 이벤트의 발생 빈도 조절 -> 가중치 -> 인사이트 등 도메인 분석을 하여 설계 -> 현재는 가정
WEIGHTS = [32, 20, 17, 9, 11, 9]
# 카테고리 정의 
...
def generate(fake:Faker, *, timezone_name:str, environment:str, run_id:str) -> dict:
  ...
  # 이벤트별 요청에 대한 method, URL(API 경로), 지연 
  routes = {
