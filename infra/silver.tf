...
resource "aws_kinesis_firehose_delivery_stream" "silver" {
  ...
  extended_s3_configuration {
    ...
    # 아래처럼 구성 -> partition pruning -> Athena/opensearch/Glue/spark 등 열 기반으로 데이터 추출 유용 
    # S3 버킷 접두사
    # silver/year=2026/month=08/day=20/hour=11/... 이렇게 파티션 가능 -> 검색 속도 빨라짐 
  ...
  # 의존성
  depends_on = [
    # 해당 정책 입력/출력 액세스 권한 생성된 후에 firehose 생성되도록 설정
