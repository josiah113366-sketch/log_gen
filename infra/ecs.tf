...
resource "aws_ecs_task_definition" "generator" {
  # 관리단 => family, 소속그룹
  # de-ai-16-loggen-task:1 => de-ai-25-loggen-task:2 => ... 수정발생 => 넘버를 부여하여 새로 생성
  ...
  # [브론즈 추가]
  # 컨테이너 내부에서 파이썬으로 로그를 생성하고 kinesis로 전송
  ...
  container_definitions = jsonencode([
    {
      ...
      environment = [
        ...
        # 로그 생성 실행 단위 구분용. manual, 20260820-001, 20260820-002, ... 의미 부여 가능
        { name = "RUN_ID", value = "manual" }
      ]

      # cloudwatch로 로그 전송 설정
      logConfiguration = {
        # cloudwatch logs 용으로 로그 드라이버 지정
        logDriver = "awslogs"
